...
from utils import format_time, peek_file_content
...
def analyze_consultant_performance(consultant_df: pd.DataFrame, calltime_df: pd.DataFrame) -> Tuple[Optional[pd.DataFrame], Optional[str]]:
    """
    상담원 실적을 분석하는 함수
...
def create_excel_report(performance_df: pd.DataFrame) -> Optional[bytes]:
    """
